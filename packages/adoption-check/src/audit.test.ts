...
test('declaring a package variable trips the namespace gate', () => {
  ...
  const { findings } = auditApp(root);
  expect(findings.some((f) => f.gate === 'namespace' && f.file === 'app/globals.css')).toBe(true);
  rmSync(root, { recursive: true, force: true });
});
...
test('restating the official palette or a retired hex trips the palette gate', () => {
  ...
  expect(palette.some((f) => f.message.includes('#fdd238'))).toBe(true);
  expect(palette.some((f) => f.message.includes('#14142b') && f.message.includes('retired'))).toBe(true);
  rmSync(root, { recursive: true, force: true });
});
...
test('zero or two theme bundles trips the bundle gate; nexus needs the compat flag', () => {
  ...
  expect(auditApp(none).findings.some((f) => f.gate === 'bundle')).toBe(true);
  ...
  expect(auditApp(both).findings.some((f) => f.gate === 'bundle')).toBe(true);
  ...
  expect(auditApp(nexus).findings.some((f) => f.gate === 'bundle')).toBe(true);
  expect(
...
test('duplicated brand fonts trip the fonts gate', () => {
  ...
  expect(auditApp(root).findings.some((f) => f.gate === 'fonts')).toBe(true);
  rmSync(root, { recursive: true, force: true });
});
...
test('styling package internals trips the internals gate', () => {
  ...
  expect(auditApp(root).findings.some((f) => f.gate === 'internals')).toBe(true);
  rmSync(root, { recursive: true, force: true });
});
...
test('exact pins and pre-2.0 ranges trip the versions gate', () => {
  ...
  expect(findings.some((f) => f.message.includes('pinned'))).toBe(true);
  expect(findings.some((f) => f.message.includes('adoption floor'))).toBe(true);
...
test('a duplicated Tailwind palette trips the tailwind gate', () => {
  ...
  expect(auditApp(root).findings.some((f) => f.gate === 'tailwind')).toBe(true);
  rmSync(root, { recursive: true, force: true });
});
...
test('the allowFiles config exempts a named file, nothing else', () => {
  ...
  const { findings } = auditApp(root);
  expect(findings.filter((f) => f.file === 'emails/branded.css').length).toBe(0);
  expect(findings.some((f) => f.file === 'app/stray.css')).toBe(true);
  rmSync(root, { recursive: true, force: true });
});
